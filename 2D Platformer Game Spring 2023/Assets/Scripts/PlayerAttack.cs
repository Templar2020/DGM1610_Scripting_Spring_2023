...
public class PlayerAttack : MonoBehaviour
{
    ...
    void Update()
    {
       if(attackDelay <= 0) // If attack delay is zero allow an attack
       {
           ...
           {
               Collider2D[] enemiesToDamage = Physics2D.OverlapCircleAll(attackPos.position, attackRange, whatIsEnemies); // Draw out an attack circle. If enemy are in the circel apply damage when attack key is pressed. 
